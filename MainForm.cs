...
namespace GraficDisplay
{
    ...
    public partial class MainForm : Form
    {
        ...
        protected void CalcDataGraphs()
        {
            ...
            for (int j = 0; j < NumGraphs; j++)
            {
                ...
                switch (CurExample)
                {
                    ...
                    case "TILED_VERTICAL":
                        this.Text = "NASARTIcs (vertical prefered)";
                    ...
                    case "TILED_VERTICAL_AUTO":
                        this.Text = "NASARTIcs (vertical prefered) autoscaled";
                        display.PanelLayout = PlotterGraphPaneEx.LayoutMode.TILES_VER;
                        display.DataSources[j].Length = 5800;
                        display.DataSources[j].AutoScaleY = true;
                        display.DataSources[j].SetDisplayRangeY(-300, 600);
                    ...
                    case "TILED_HORIZONTAL":
                        this.Text = "NASARTIcs (horizontal prefered)";
                    ...
                    case "TILED_HORIZONTAL_AUTO":
                        this.Text = "NASARTIcs (horizontal prefered) autoscaled";
                        display.PanelLayout = PlotterGraphPaneEx.LayoutMode.TILES_HOR;
                        display.DataSources[j].Length = 5800;
                        display.DataSources[j].AutoScaleY = true;
                        display.DataSources[j].SetDisplayRangeY(-300, 600);
        ...
        private void LunarSurfaceToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // zzz
            //    Extend current chromosome aberration model, validating model results against in vitro exposure results, and use
            // validated model to simulate and predict effects of lunar surface exposure.
            // reference beam
        }
    }
}